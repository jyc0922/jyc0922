...
export default function SignUp() {
    const [{email, password, confirmPassword}, setForm] = 
      ...
      const createAccount = useCallback(() => {
        ...
        } else alert ('password is not same 비밀번호가 일치하지 않습니다.')
      ...
      <div className="flex flex-col min-h-screen bg-gray-100 border border-gray-300 shadow-xl rounded-xl">
        <div className="flex flex-col items-center justify-center flex-1 max-w-sm px-2 mx-auto">
          <div className="w-full px-6 py-8 text-black bg-white rounded shadow-md">
            ...
            <h1 className="mb-8 text-2xl text-center text-primary">Sign Up회원가입</h1>

            <input type="text" className="w-full p-3 mb-4 input input-primary"
             name="email" placeholder="Email 입력하세요" value={email} onChange={changed('email')}/>
            <input type="password" className="w-full p-3 mb-4 input input-primary"
             name="password" placeholder="Password 입력하세요" value={password} onChange={changed('password')}/>
            <input type="password" className="w-full p-3 mb-4 input input-primary"
             name="confirm_password" placeholder="Confirm Password 비밀번호 확인하세요" 
          ...
          <div className="mt-6 text-grey-800">
            Already have an account 계정을 보유 중이신가요?
