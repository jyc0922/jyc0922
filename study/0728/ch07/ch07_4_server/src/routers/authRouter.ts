  ...
  return router
    .post('/signup', async (req, res) => {
      ...
      try {
        ...
        if (exists) {
          res.json({ok: false, message: 'Already exists!, 이미 가입한 회원입니다.'})
        } else {
        ...
        if (e instanceof Error) res.json({ok: false, message: e.message})
      }
    })
      ...
      try {
        ...
        if (tmp.length !== 2)
          ...
          if (email !== result.email) {
            res.json({
              ok: false,
              errorMessage: '이메일이 일치하지 않습니다, 다시 확인 하세요.'
            })
            return
          }
          ...
          if (false === same) {
            res.json({
              ok: false,
              errorMessage: '비밀번호가 일치하지 않습니다, 다시 확인 하세요.'
            })
            return
          }
