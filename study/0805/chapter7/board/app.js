...
app.post("/write-comment", async (req, res) => {
    ...
    if (post.comments) {
        post.comments.push({ idx: post.comments.length + 1, name, password, comment, 
            createdDt: new Date().toISOString() });
    } else {
        post.comments = [{ idx: 1, name, password, comment, 
            createdDt: new Date().toISOString() }];
    }
...
app.delete('/delete-comment', async (req, res) => {
    ...
    const post = await collection.findOne(
        {_id: new ObjectId(id), comments: {$elemMatch: {idx: parseInt(idx), password}}
